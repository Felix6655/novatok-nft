...
export default function Navbar() {
  ...
  const projectId = typeof window !== 'undefined' ? process.env.NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID : '';
  const isWalletConnectConfigured = Boolean(projectId);
              ...
              <>
                {isWalletConnectConfigured ? (
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-green-500/20 border border-green-500/30">
                    ...
                    <span className="text-sm text-green-300">Sepolia Ready</span>
                  </div>
                ) : (
