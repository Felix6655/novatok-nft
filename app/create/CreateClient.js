...
export default function CreateClient() {
  ...
  const PLACEHOLDER_IMG = 'https://placehold.co/512x512?text=No+Image';
  const [imageLoadStatus, setImageLoadStatus] = useState({}); // { [id]: 'ok' | 'error' | 'loading' }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!isContractConfigured) {
        demo = true;
        const fallback = DEMO_IMAGES[Math.floor(Math.random() * DEMO_IMAGES.length)];
        urls = [fallback];
        ...
        urls = (data.images || [data.imageUrl]).filter(Boolean);
        toast.success('Image generated successfully!');
      }
      // Filter out empty/undefined URLs
      urls = urls.filter(url => url && typeof url === 'string' && url.trim() !== '');
      setGeneratedImages((prev) => {
        const arr = [...urls, ...prev];
        console.log('Final images array:', arr);
        return arr;
      });
      setIsDemo(demo);
    } catch (err) {
        ...
        {generatedImages.length > 0 && (
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {generatedImages.map((url, idx) => (
              <GlassCard key={url + idx} className="p-4 flex flex-col items-center" hover={true}>
                <div className="relative w-full aspect-square rounded-xl overflow-hidden mb-4 bg-white/5 flex items-center justify-center">
                  <img
                    src={url}
